...
# Ensure backend root is on sys.path so `src` package imports work when this
# file is executed directly from `backend/src` (common during development).
backend_root = Path(__file__).resolve().parents[1]
if str(backend_root) not in sys.path:
    sys.path.insert(0, str(backend_root))

# Load .env explicitly from backend root so GEMINI_API_KEY is available
try:
...
CACHE_FILE = str(backend_root / "cache" / "scholar_cache.json")
